    ...
    public static void main(String[] args) throws Exception {
        ...
        for(int i=0; i<args.length; i++){
            try{
                ...
                TypeCheckerVisitor check = new TypeCheckerVisitor(eval.initClass, eval.varsInClass, eval.funcsInClass, eval.varsInFuncs);
                root.accept(check, null);

                System.err.println("File " + args[i] + GREEN + " PASS" + RESET);
                eval.printOffsets(eval.initClass);
            }
                ...
                System.err.println("File " + args[i] + RED + " FAIL" + RESET + "\n");
            }
            finally{
                try{
...
class Offsets {
    ...
    void increaseVarOffset(int value) { this.varOffset += value; }
...
class DeclarationVisitor extends GJDepthFirst<String, List<String>>{ //0st element = classname -> 1nd element = funcName for varFunc
    ...
    LinkedHashMap<String, LinkedHashMap<String, Integer>> offsetTableVars = new LinkedHashMap<String, LinkedHashMap<String, Integer>>(); //storing the vars offsets
    ...
    public void printOffsets(LinkedHashMap<String, String> map){

        for (Map.Entry<String, String> classes : map.entrySet()) {

            String className = classes.getKey();
            offsetTableVars.put(className, new LinkedHashMap<String, Integer>());
            ...
            if(varsInClass.containsKey(className)){
                ...
                for (Map.Entry<String, String> var : vars.entrySet()) {
                    ...
                    offsetTableVars.get(className).put(var.getKey(), offsetTableCounter.get(className).getVarOffset());
                    System.out.println(className + "." + var.getKey() + " : " + offsetTableCounter.get(className).getVarOffset());

                    if(var.getValue().equals("int"))
                        offsetTableCounter.get(className).increaseVarOffset(4);
                    else if(var.getValue().equals("boolean"))
                        offsetTableCounter.get(className).increaseVarOffset(1);
                    else
                        offsetTableCounter.get(className).increaseVarOffset(8);
